...
const VeHuyChart = (props) => {
    ...
    useEffect(() => {
        if (props.opt && props.opt.diaDiem) {
            setChartTitle('Thống kê số vé hủy theo ngày')
            setChartType(0)
        }
    }, [props.opt])
    ...
    function getPieData(data) {
        ...
        data.forEach((e) => {
            if (e.receiptStatistic && e.receiptStatistic[0]) {
                let obj = new Object()
                obj.name = e.name
                obj.y = e.receiptStatistic[0].soVe
                obj.tip = e.receiptStatistic[0].doanhThu / 1000000
                arrResult.push(obj)
            }
        })
        return arrResult
    ...
    const options = {
        ...
        tooltip: {
            ...
            pointFormat:
                '<tr><td style="padding:0"><b>{point.y} vé</b></td></tr>' +
                '<tr><td style="padding:0"><b>{point.tip:.3f} triệu đồng</b></td></tr>',
            footerFormat: '</table>',
            shared: true,
            useHTML: true,
        },
