...
import { fetchDoiTuongLoaiVe, thongKeVeHuy } from '../store/thongke-action'
...
const ThongKeVeHuy = (props) => {
    ...
    const onClick = () => {
        ...
        if (diaDiemTag.length == 0) {
            ...
            dispatch(thongKeVeHuy(df, dt, diaDiemTag, 0))
        } else if (
            loaiKhachBieuDo == 1 &&
            diaDiemTag.length > 0 &&
            ...
            dispatch(thongKeVeHuy(df, dt, diaDiemTag, loaiKhachTag))
        }
    }
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            if (currentDiaDiem == null) {
                dispatch(thongKeVeHuy(null, null, null, null))
            } else if (currentPhanLoaiKhach == 0) {
                dispatch(
                    thongKeVeHuy(
                        currentDateFrom,
                        currentDateTo,
